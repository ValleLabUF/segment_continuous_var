...
#   
  ...
  addPolylines(lng = as.numeric(st_coordinates(df())[,1]),
               ...
               color = "darkturquoise",
               opacity = 0.8) %>% 
  addCircleMarkers(data = df.pts(),
                   lng = as.numeric(st_coordinates(df.pts())[1,1]),
                   lat = as.numeric(st_coordinates(df.pts())[1,2]),
                   fillColor = "#5EF230",
                   stroke = FALSE,
                   fillOpacity = 0.8) %>% 
  addCircleMarkers(data = df.pts(),
                   lng = as.numeric(st_coordinates(df.pts())[2,1]),
                   lat = as.numeric(st_coordinates(df.pts())[2,2]),
                   fillColor = "red",
                   stroke = FALSE,
                   fillOpacity = 0.8)
