...
Click the __View all tracks__ tab to view tracks for all individuals. This provides a quick overview of all track segments separately and allows for filtering tracks by different variables.  
...
#   
  ...
  clearControls() %>%
  clearShapes() %>%
  fitBounds(as.numeric(st_bbox(df())[1]),
            as.numeric(st_bbox(df())[2]),
            as.numeric(st_bbox(df())[3]),
            as.numeric(st_bbox(df())[4])) %>%
  addPolylines(lng = as.numeric(st_coordinates(df())[,1]),
               lat = as.numeric(st_coordinates(df())[,2]),
               weight = 2,
               color = "darkturquoise",
               opacity = 0.5) #%>% 
  ...
  #                    slice(1))
                   # radius = ~3,
                   # fillColor = "#BFBFBF", #rgb(t(col2rgb("grey75")), maxColorValue = 255)
                   # stroke = FALSE,
                   # fillOpacity = 0.5)
})
